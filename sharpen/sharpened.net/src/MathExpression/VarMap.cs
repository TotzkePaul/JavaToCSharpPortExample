...
namespace MathExpression
{
	...
	public class VarMap
	{
		...
		/// <remarks>Returns the value associated with the specified variable name.</remarks>
		/// <exception cref="Sharpen.RuntimeException">If a matching variable name cannot be found.
		/// 	</exception>
		public virtual double GetValue(string varName)
		{
			for (int i = 0; i < numVars; i++)
			{
				if (caseSensitive && name[i].Equals(varName) || !caseSensitive && Sharpen.Runtime.EqualsIgnoreCase
					(name[i], varName))
				{
					return value[i];
				}
		...
		/// <remarks>Assigns the value to the specified variable name.</remarks>
		...
		/// <remarks>Returns true if the case of the variable names is considered.</remarks>
		public virtual bool IsCaseSensitive()
		{
			return caseSensitive;
		}

		/// <summary>Returns an array of exact length of the variable names stored in this map.
		/// 	</summary>
		/// <remarks>Returns an array of exact length of the variable names stored in this map.
		/// 	</remarks>
		...
		/// <remarks>Removes the variable-name from the map. Does nothing if the variable-name is not found.
		/// 	</remarks>
